...
{
	Effect effect;
	const int EFFECTASZX = 300;
	const int EFFECTASZY = 420;
	CreateOneEffect(effect.pos, (float)x, (float)y, 0.f, 0.f,
		"ora0", 0, EFFECTASZX, EFFECTASZY);
...
bool StepEffect_FadeInBlack(Effect& effect)
{
  charabase::CharaBase& pos = effect.pos;
  //おおよそ30フレームで消滅
  pos.a -= 255.f * system::ONEFRAME_TIME*2.f;
  if (pos.a <= 0.f){
    pos.a = 0.f;
    return false;
  }
...
//カーテン
void CreateEffect_CurtainOpen(int winw, int winh, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };

	const int EFFECTASZX = 640;
	const int EFFECTASZY = 480;


	e = zero;
	//CreateOneEffect(e.pos, system::WINW/2.0f, system::WINH/2.0f, 0, 0,
	//	"curtainOpen", 0, EFFECTASZX, EFFECTASZY, 0, 0.f, float(system::WINW) / float(EFFECTASZX), float(system::WINH) / float(EFFECTASZY));
	CreateOneEffect(e.pos, system::WINW / 2.0f, system::WINH / 2.0f, 0, 0,
		"curtainOpen", 0, EFFECTASZX, EFFECTASZY, 0, 0.f,1.5f,1.5f);
	e.pos.pos.z = 0.0f;
	e.type = type;
	e.renderMode = NORMAL;
	e.repertDraw = 1;
...
bool StepEffect_CurtainOpen(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
void CreateEffect_CurtainClose(int winw, int winh, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };
	...
	e = zero;
	//CreateOneEffect(e.pos, system::WINW / 2.0f, system::WINH / 2.0f, 0, 0,
	//	"curtainClose", 0, EFFECTASZX, EFFECTASZY, 0, 0.f, float(system::WINW) / float(EFFECTASZX), float(system::WINH) / float(EFFECTASZY));
	CreateOneEffect(e.pos, system::WINW / 2.0f, system::WINH / 2.0f, 0, 0,
		"curtainClose", 0, EFFECTASZX, EFFECTASZY, 0, 0.f, 1.5f,1.5f);
	e.pos.pos.z = 0.0f;
	e.type = type;
	e.renderMode = NORMAL;
	e.repertDraw = 1;
...
bool StepEffect_CurtainClose(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
void CreateEffect_CurtainCloseOpen(int winw, int winh, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };
...
bool StepEffect_CurtainCloseOpen(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
struct EffectFunc{
	...
	{ CreateEffect_CurtainOpen, StepEffect_CurtainOpen },
	{ CreateEffect_CurtainClose, StepEffect_CurtainClose },
	{ CreateEffect_CurtainCloseOpen, StepEffect_CurtainCloseOpen },
};
...
void effect::LoadReses()
{
  ...
  graph::Draw_LoadObject("curtainOpen", "res/gra/effect/curtain.png");
  graph::Draw_LoadObject("curtainClose", "res/gra/effect/curtain2.png");
