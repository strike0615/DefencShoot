...
//プロミネンス利用P2　攻撃
void CreateEffect_P2(int x, int y, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };
...
//プロミネンス
//Stepカスタマイズ
bool StepEffect_P2(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
//プロミネンス利用P3 爆発
void CreateEffect_P3(int x, int y, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };
...
//プロミネンス
//Stepカスタマイズ
bool StepEffect_P3(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
//プロミネンス利用P4　ダッシュ
void CreateEffect_P4(int x, int y, effect::EFFECTTYPE type)
{
	Effect e;
	Effect zero = { 0 };
...
//プロミネンス
//Stepカスタマイズ
bool StepEffect_P4(Effect& effect)
{
	effect.animCnt++;
	if (effect.animCnt >= effect.animFrm){
		effect.animCnt = 0;
...
struct EffectFunc{
	...
	{ CreateEffect_P2, StepEffect_P2 },
	{ CreateEffect_P3, StepEffect_P3 },
	{ CreateEffect_P4, StepEffect_P4 },
};
...
void effect::LoadReses()
{
	...
	graph::Draw_LoadObject("ora0", "res/gra/effect/ora0.png");
